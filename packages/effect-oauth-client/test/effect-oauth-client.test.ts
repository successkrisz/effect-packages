...
import { Context, Duration, Effect, Layer, ManagedRuntime, Redacted, Schema, TestClock, TestContext } from "effect"
...
const makeService1 = (creds: OAuthClient.Credentials) =>
  Effect.gen(function* () {
    ...
    const getSecretFoo = () =>
      client
        .get(
          `https://api.example.com/secret-foo`
        )
        .pipe(
...
describe("OAuthClient", () => {
  ...
  beforeEach(() => {
    ...
    rt = ManagedRuntime.make(SomeService1Layer.pipe(Layer.provideMerge(TestLayer), Layer.provide(TestContext.TestContext)))
  })
  ...
  it("Should only call the token endpoint once", async () => {
    fetch.mockImplementation(async (url) => {
      if (url.href.includes("token")) {
        return new Response(
          JSON.stringify({ access_token: "test", expires_in: 3600, token_type: "Bearer" }),
          { status: 200 }
        )
      }
